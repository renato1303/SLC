...
export default function App() {
  // IntersectionObserver for lightweight scroll reveal transitions (unobserves once visible to ensure smooth mobile scrolling)
  useEffect(() => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // Unobserve element once visible to prevent re-triggering animation and re-layout during touch scroll
  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A] text-[#F5F7FA] font-sans selection:bg-[#22D3A6] selection:text-[#0B0F1A]">
      {/* 1. Header (Logo + CTA, No nav menu links) */}
      ...
      <main>
        ...
        {/* 3. Formulário + Seção "Antes de qualquer proposta" (2 colunas lado a lado) */}
